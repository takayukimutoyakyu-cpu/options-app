...
with tab4:
    ...
    # --- 株取引が初めての方へ ---
    st.markdown("### 🔰 株取引が初めての方へ")

    with st.expander("「株を買う」ってどういうこと？"):
        st.markdown("""
株を買うとは、**企業の一部を所有すること**です。

例えばアップル（AAPL）の株を1株買うと、あなたはアップルの「株主」になります。
株価が上がれば利益、下がれば損失になります。

米国株は **1株から** 購入できます（日本株のように100株単位ではありません）。
""")

    with st.expander("「成行注文」と「指値注文」の違いは？"):
        st.markdown("""
**成行注文（なりゆき）** — 「今すぐ買いたい！」
...
**指値注文（さしね）** — 「この値段になったら買いたい」

- 自分で「○○ドル以下なら買う」と価格を指定します
- メリット：希望の価格で取引できる
- デメリット：その価格にならなければ取引が成立しない

💡 オプション取引でも同じです。慣れないうちは **指値注文** で価格を確認しながら取引するのが安心です。
""")

    with st.expander("「ティッカー」って何ですか？"):
        st.markdown("""
ティッカーとは、**株を識別するための短い英字コード**のことです。
...
このアプリの作戦ナビでは、ティッカーを入力して銘柄を検索します。日本語（例：「テスラ」「アップル」）でも検索できます。
""")

    with st.expander("「1枚」「1契約（コントラクト）」って何ですか？"):
        st.markdown("""
オプション取引では、**1枚＝株100株分** の権利です。

例えば「プレミアム $3.00」のオプションを1枚買うと：
- 支払い金額 = **$3.00 × 100 = $300**

1枚売ると：
- 受け取り金額 = **$3.00 × 100 = $300**

⚠️ 証券会社の画面では「1」と入力すれば1枚（100株分）になります。「100」と入れると100枚（10,000株分）になってしまうので注意！
""")

    with st.expander("取引時間はいつですか？"):
        st.markdown("""
**米国市場の取引時間**（日本時間）：
...
- プレマーケット（開場前）やアフターマーケット（閉場後）でも取引できるブローカーもあります
- オプションの満期日は通常 **金曜日** です（アメリカ時間）
""")
